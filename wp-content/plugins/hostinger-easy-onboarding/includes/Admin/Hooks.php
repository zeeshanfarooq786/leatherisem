...
class Hooks
{
    ...
    public function __construct()
    {
        ...
        add_action('admin_footer', [$this, 'rate_plugin']);

        // Admin init actions
        add_action('admin_init', [$this, 'admin_init_actions']);
        add_action('admin_init', [$this, 'set_woocommerce_options'], 0);

        // Admin notices
        add_action('admin_notices', [$this, 'omnisend_discount_notice']);

        // Return back to onboarding.
        add_action('admin_footer', [$this, 'back_to_onboarding_notice']);
        add_filter('admin_body_class', [$this, 'add_onboarding_notice_class']);
        ...
        add_action('admin_menu', [$this, 'disable_beaver_builder_redirect']);

        add_action('admin_menu', [$this, 'disable_monsterinsights_redirect']);
    ...
    public function check_plugins_and_remove_duplicates()
    {
        ...
        if (is_plugin_active('astra-sites/astra-sites.php') && is_plugin_active('woocommerce/woocommerce.php')) {
            ...
            $page_slugs       = ["shop", "cart", "checkout", "my-account"];
            $astra_page_slugs = ["shop-2", "cart-2", "checkout-2", "my-account-2"];
            $pages_to_remove  = [];
            $pages_to_rename  = [];
    ...
    public function omnisend_discount_notice(): void
    {
        ...
        if ($omnisend_notice_hidden === false
            && $this->helper->is_this_page('/wp-admin/admin.php?page=omnisend')
            && (Helper::is_plugin_active('class-omnisend-core-bootstrap')
                || Helper::is_plugin_active(
            ...
            <div class="notice notice-info hts-admin-notice hts-omnisend">
                <p><?php
                    echo wp_kses(
                        __(
                            'Use the special discount code <b>ONLYHOSTINGER30</b> to get 30% off on Omnisend for 6 months when you upgrade.',
    ...
    public function hide_plugin_panel(string $plugin_slug, $panel_id)
    {
        ...
        if (empty($persisted_preferences)) {
            ...
            if ( ! empty($persisted_preferences['core/edit-post']['inactivePanels'])
                 && ! in_array(
                    $panel_id,
                    $persisted_preferences['core/edit-post']['inactivePanels']
                )) {
            ...
            <div class="hostinger-onboarding-reminder">
                <div class="hostinger-onboarding-reminder__wrap">
                    ...
                    <div class="hostinger-onboarding-reminder__text">
                        <?php
                        echo __('Just a few more steps to launch your online store', 'hostinger-easy-onboarding'); ?>
                    </div>
                    <div class="hostinger-onboarding-reminder__cta">
                        <a href="<?php
                        echo admin_url('admin.php?page=hostinger'); ?>">
                            <?php
                            echo __('Continue setup', 'hostinger-easy-onboarding'); ?>
                        </a>
                    </div>
                </div>
            </div>
